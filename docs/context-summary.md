...
- **Special characters (e.g. ?quinoxe):** `TagTextHelper.Clean` fixes Latin-1 mojibake and falls back to the on-disk filename when tag title contains U+FFFD. `AudioTagReader` uses it for artist/album/title. Existing library may need **Rescan library** to refresh SQLite titles.
...
## 2026-08-01 ? Global media Play/Pause keys (v0.13.22)

- **User:** Keyboard Play/Pause does not control LMH.
- **Cause:** `GlobalMediaKeyService` only handled `WM_APPCOMMAND` on the MainWindow HWND ? keys reach the foreground window only.
- **Fix:** Register global hotkeys for VK_MEDIA_PLAY_PAUSE / Next / Prev / Stop; keep APPCOMMAND for volume and as fallback if registration fails (skip duplicate when hotkey registered). `HandleMediaPlayPause` matches toolbar (start selection when idle).
- **Build:** Synced **0.13.22** to `%LocalAppData%\Programs\LocalMusicHub\`.

## 2026-08-01 ? Publishing v0.13.22
